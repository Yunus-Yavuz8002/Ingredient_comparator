...
from Ingredient_comparator import Product
...
Enter your ingredients:cold


How much/many cold do you have?:20


Enter the unit of cold:glasses
...
Template method pattern is used in Hazirlik(preparation in Turkish) and Product in Ingredient_comparator.py
...
1. Abstract Base Classes (ABCs): Hazirlik class inherits from abc.

2. Encapsulation & Mutability Protection:Inside the Hazirlik.__init__ constructor, defensive copying (self.malzeme = malzeme.copy()) protects the internal dictionary structure.

3. Inheritance & Polymorphism:Product class inherits from abstract base Hazirlik class and uses super()__init__ to initialize its attributes

4. Robust Error Handling:In malzeme_listesi_olusturma(self), a try-except loop captures ValueError for non-float(like strings) values.

5. Separation of Concerns (SoC):Ratio class calculates for Ingredient_comparator

6. Clean Type Annotation & Type Safety:Type hinting standarts are used for almost every function
